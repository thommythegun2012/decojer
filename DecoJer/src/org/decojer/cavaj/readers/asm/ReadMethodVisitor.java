...
public class ReadMethodVisitor extends MethodVisitor implements ReadVisitor {
	...
	public ReadMethodVisitor(@Nonnull final ReadClassVisitor parentVisitor) {
		...
		this.annotationVisitor = new ReadAnnotationMemberVisitor(this);
	}

	private final void add(final Op op) {
		this.ops.add(op);
		if (this.insnAnnotationInfo != null && this.insnAnnotationInfo.a != null) {
			// JDK 8.0 has +1 index to Eclipse! who is wrong? JDK or Eclipse? we try both...
	...
	public void visitAttribute(final Attribute attr) {
		if (!attr.type.equals("org.aspectj.weaver.MethodDeclarationLineNumber")) {
	...
	public void visitFieldInsn(final int opcode, final String owner, final String name,
		...
		if (owner == null || name == null || desc == null) {
			log.warn(getM() + ": Cannot read get operation with field '" + owner + "." + name
					+ "' and descriptor '" + desc + "'!");
			return;
		}
		switch (opcode) {
	...
	public void visitMethodInsn(final int opcode, final String owner, final String name,
		...
		if (owner == null || name == null || desc == null) {
			log.warn(getM() + ": Cannot read invoke operation with method name '" + owner + "."
					+ name + "' and descriptor '" + desc + "'!");
			return;
		}
		switch (opcode) {
		/**********

 ...
 */
@Slf4j
public final class TrOperations {
	...
	public static void transform(@Nonnull final CFG cfg) {
		new TrOperations(cfg).transform();
	}
	...
	private TrOperations(@Nonnull final CFG cfg) {
		this.cfg = cfg;
	}
	...
	private void transform() {
		...
		for (int i = bbs.size(); i-- > 0;) {
			...
			if (!transformCatchExpression(bb)) {
	...
	private boolean transformCatchExpression(@Nonnull final BB bb) {
		final E catchIn = bb.getCatchIn();
		if (catchIn == null || bb.getStmts() > 0 || bb.getTop() > 0) {
			// TODO some rewrites combine join nodes and push called twice, stmts/top check not
			// sufficient
			return false;
		}

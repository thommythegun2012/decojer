...
public class FrameworkProperties {
	...
	// Many existing plugins get framework propeties directly from System
	// instead of BundleContext.
	// Note that the OSGi TCK is one example where this property MUST be set to
	// false because many TCK bundles set and read system properties.
	...
	public static String decode(String urlString) {
		...
		try {
			final Class<? extends URLDecoder> clazz = URLDecoder.class;
			...
			final Object result = method.invoke(null, new Object[] { urlString,
					"UTF-8" }); //$NON-NLS-1$
			if (result != null) {
		...
		final byte[] encodedBytes = urlString.getBytes();
		final int encodedLength = encodedBytes.length;
		final byte[] decodedBytes = new byte[encodedLength];
		int decodedLength = 0;
		for (int i = 0; i < encodedLength; i++) {
			byte b = encodedBytes[i];
			if (b == '%') {
				final byte enc1 = encodedBytes[++i];
				final byte enc2 = encodedBytes[++i];
	...
	private static int hexToByte(final byte b) {
		switch (b) {
		case '0':
			return 0;
		case '1':
	...
	public static void initializeProperties() {
		...
		if (getProperty(PROP_FRAMEWORK) == null
				|| getProperty(PROP_INSTALL_AREA) == null) {
			final CodeSource cs = FrameworkProperties.class
					.getProtectionDomain().getCodeSource();
			if (cs == null) {
				throw new IllegalArgumentException(NLS.bind(
						EclipseAdaptorMsg.ECLIPSE_STARTUP_PROPS_NOT_SET,
						PROP_FRAMEWORK + ", " + PROP_INSTALL_AREA)); //$NON-NLS-1$
			}
	...
	private static synchronized Properties internalGetProperties(
			String usingSystemProperties) {
		if (properties == null) {
			final Properties systemProperties = System.getProperties();
			if (usingSystemProperties == null) {
				usingSystemProperties = systemProperties
						.getProperty(USING_SYSTEM_PROPERTIES_KEY);
			}
			if (usingSystemProperties == null
					|| usingSystemProperties.equalsIgnoreCase(Boolean.TRUE
							.toString())) {
				...
				// snapshot of System properties for uses of getProperties who
				// expect to see framework properties set as System properties
				// we need to do this for all system properties because the
				// properties object is used to back
				// BundleContext#getProperty method which expects all system
	...
	public static synchronized void setProperties(
		...
		if (input == null) {
			// just use internal props; note that this will reuse a previous set
			// of properties if they were set
			internalGetProperties("false"); //$NON-NLS-1$
			return;
		}
		properties = null;
		final Properties toSet = internalGetProperties("false"); //$NON-NLS-1$
